...
export function expectedItemFields(announcementsResponse: any) {
    if (announcementsResponse.data.items != null && announcementsResponse.data.items.length > 0) {
        announcementsResponse.data.items.forEach((item: any) => {
            ...
            expect(item.startDate === null || typeof item.startDate === 'string').toBeTruthy();
            if(item.startDate){
                expect(Date.parse(item.startDate)).not.toBeNaN();
            }
            expect(item.endDate === null || typeof item.endDate === 'string').toBeTruthy();
        ...
        throw new Error("No announcements found. The test cannot continue.'Get all the announcements'");
    }
}

// Función para crear un comunicado de prueba
export async function createAnnouncementForTest(request: APIRequestContext): Promise<number> {
